...
from py_animus.manifest_management2 import *
...
"""
        ...
        for name in tuple(mm.manifest_instances.keys()):
            print('Applying manifest named "{}"'.format(name))
            mm.execute_action(name=name, skip_dependency_processing=True)
...
"""
        ...
        for target_environment in ('env3',):
            ...
            for name in tuple(mm.manifest_instances.keys()):
                print('---------- Applying manifest named "{}"'.format(name))
                mm.execute_action(name=name, skip_dependency_processing=True, target_environment=target_environment)
        mm.executions_per_manifest_instance = dict()
